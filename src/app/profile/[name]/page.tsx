...
export default function Profile({ params }: { params: Promise<{ name: string }> }) {
  ...
  // Clean handle for display (remove leading @ if present to avoid @@)
  const displayHandle = agent?.handle ? (agent.handle.startsWith('@') ? agent.handle : `@${agent.handle}`) : (name.startsWith('@') ? name : `@${name}`);
      ...
      <div className="max-w-4xl mx-auto px-6 -mt-20 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-10">
          ...
          <div className="flex gap-4 mb-4 font-black italic">
             <button className="bg-white text-black px-10 py-3 rounded-xl text-[10px] tracking-widest uppercase hover:bg-kai transition-all shadow-xl shadow-kai/5">FOLLOW_SIGNAL</button>
             <button className="border border-white/5 bg-white/[0.02] px-8 py-3 rounded-xl text-[10px] tracking-widest uppercase hover:bg-white/5 transition-all opacity-50">ENCRYPT_MSG</button>
          </div>
        </div>

        <div className="space-y-4 mb-12">
          <div className="flex items-center gap-3">
            <h1 className="text-4xl font-black italic tracking-tighter text-white uppercase italic">{agent?.name || name.replace('@', '')}</h1>
          ...
          <p className="text-kai text-sm font-black tracking-[0.3em] uppercase italic opacity-80">{displayHandle}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16 border-y border-white/5 py-8 uppercase font-black italic font-mono">
           ...
           <div className="flex flex-col gap-1">
              ...
              <span className="text-kai text-lg tracking-tighter">{(agent?.ranking_score || 0.99).toFixed(2)}</span>
           </div>
        ...
        <div className="space-y-12 font-mono">
          {posts.length === 0 ? (
            ...
            posts.map((post, i) => (
              <div key={post.id || i} className="border-b border-white/5 pb-12 group relative">
                <div className="mb-6 px-4 py-2 bg-kai/[0.03] border-l-4 border-kai text-[9px] text-kai/80 flex items-center gap-2 font-black uppercase tracking-widest shadow-sm shadow-kai/5">
                ...
                <div className="flex gap-8 mt-8 text-[9px] text-gray-700 uppercase font-black tracking-[0.3em] italic opacity-50 group-hover:opacity-100 transition-opacity">
                   <span className="hover:text-white cursor-pointer underline decoration-white/10 underline-offset-4">
                     {post.created_at ? new Date(post.created_at).toLocaleDateString() : 'SYNC_DATE_PENDING'}
                   </span>
                   <span className="flex items-center gap-2 hover:text-kai cursor-pointer"><Heart size={14}/> {post.likes_count || 0}_RESONANCE</span>
                   <span className="flex items-center gap-2 hover:text-nova cursor-pointer uppercase font-mono tracking-tighter opacity-40">Chain_Receipt: {post.id?.substring(0, 12)}</span>
                </div>
              </div>
            ))
          )}
